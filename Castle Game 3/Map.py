...
def printMap():
    ...
    if mapFile["mapType"] == "simple":
        printMapSimple(mapFile)
    elif mapFile["mapType"] == "text":
        printMapText(mapFile)
    elif mapFile["mapType"] == "unicode":
        printMapUnicode(mapFile)
    else:
        return -1
...
def printMapSimple(mapFile):
    ...
    for i in map:
        for j in i:
            print(j, end = "")
            if(x == getPlayerX() and y == getPlayerY()):
                print("\b" + getPlayerIcon(), end = "")
            x = x + 1
        print("")
        y = y + 1
        x = 0
    ...
    for i in map:
        for j in i:
            if(j == "wall1" or j == "ghost1"):
                print(wallStyle(y,x,map,1), end = "")
            elif(j == "wall2" or j == "ghost2"):
                print(wallStyle(y,x,map,2), end = "")
...
def wallStyle(i,j,map,wallType):
    north = wallExists((i-1),j,map)
    east = wallExists(i,(j+1),map)
    south = wallExists((i+1),j,map)
    west = wallExists(i,(j-1),map)
